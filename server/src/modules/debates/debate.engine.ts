...
export class DebateEngine {
  ...
  async generateAdvocateResponse(
    ...
    const agent =
      await getActiveAgentByRole(
        "advocate",
      );

    if (!agent) {
      throw new Error(
        "No active advocate agent found",
      );
    }
  ...
  async generateChallengerResponse(
    debateId: string,
  ): Promise<DebateMessage> {
    const debate =
      await getDebateById(
...

{
  ...
  "feedback": ""
    ...
    await createEvaluation({
      ...
      scenarioId:
        debate.scenarioId.toString(),

      overallScore:
        completeEvaluation.overallScore,

      reasoningScore:
        completeEvaluation.reasoningScore,

      evidenceScore:
        completeEvaluation.evidenceScore,

      counterArgumentScore:
        completeEvaluation.counterArgumentScore,

      consistencyScore:
        completeEvaluation.consistencyScore,

      adaptabilityScore:
        completeEvaluation.adaptabilityScore,

      strengths:
        completeEvaluation.strengths,

      weaknesses:
        completeEvaluation.weaknesses,

      feedback:
        completeEvaluation.feedback,
    });
...
`;
  ...
  > {
    ...
    return {
      ...
      reasoningScore:
        data.reasoningScore as number,

      evidenceScore:
        data.evidenceScore as number,

      counterArgumentScore:
        data.counterArgumentScore as number,

      consistencyScore:
        data.consistencyScore as number,

      adaptabilityScore:
        data.adaptabilityScore as number,
